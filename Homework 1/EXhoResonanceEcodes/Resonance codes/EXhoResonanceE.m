...
title('Johanned Irnawan: Equivalencies of harmonic oscillator "resonance" frequency response');
...
if (1==1)
    figure(5); clf;
    subplot(221); hI1= plot(tW,yIpS,'LineWidth',2); hold on; grid on; xlabel('Time [s]'); ylabel('x');
    title('Impulse response (no drive; P.w=0, P.p0=1, P.v0=0'); xlim([0 tW(round(numel(tW)/3))]);
    hI1b= plot(tW,exp(tW*real(lambdaP)),'r-');   % include decay envelope from eigenvalue
    hI1c= plot(tW,yEPr,'g.','MarkerSize',2);   % plot based just on eigenvalues
    legend([hI1 hI1b hI1c],'Impulse resp (yIp)','Eigenval. envelope','Eigenvalue sol.');
    subplot(222); hI2= plot(freq,db(specIp),'LineWidth',2); grid on; hold on; ylabel('Amplitude [dB]');
    title('Transf. funct. (mag. of FFT of yIp (not yIv))'); xlim(P.wDrive/(2*pi));
    subplot(224); hI3= plot(freq,angle(specIv)/(2*pi),'LineWidth',2);  grid on; hold on;
    xlabel('Frequency [Hz]'); ylabel('Phase [cycles]');
    title('Transfer function (phase of FFT of IR)'); xlim(P.wDrive/(2*pi));
    subplot(223); hZa= plot(wDT,abs(Y),'k-'); grid on; hold on;  hZb= plot(wDT,abs(Z),'r.');
    ...
    offset= max(db(Y))- max(db(specIp)); % scaling (in dB) to match up
    magY= (db(Y)- offset);
    subplot(222); hI2b= plot(wDT/(2*pi),magY,'r--','Linewidth',2);
    hI2c= plot(freq,db(specIv),'g--','LineWidth',1);
    legend([hI2 hI2b hI2c],'TF (re yIp)','(scaled) Admittance','TF (re yIv)','Location','SouthWest');
    angleY= angle(Y)/(2*pi) - angle(Y(1))/(2*pi);   % there will be a slight vert. ofseet re angle(specI)/(2*pi)
    subplot(224); hI3b= plot(wDT/(2*pi),angleY,'r--','Linewidth',2);
    hI3c= plot(freq,angle(specIp)/(2*pi),'g--','LineWidth',1);
end
...
if (1==0)
    ...
    yEMr= real(exp(tW*lambdaM)); yEMi= imag(exp(tW*lambdaM));
    figure(6); clf; subplot(221);
    hEig1= plot(tW,yEPr,'b','LineWidth',3); hold on; grid on;
    hEig2= plot(tW,yEPi,'r--','LineWidth',1); hEig3= plot(tW,yEMr,'m.','MarkerSize',8);
    hEig4= plot(tW,yEMi,'g-.','LineWidth',1); ylabel('x (eigensolution)'); xlabel('Time [s]');
    ...
    xlim([0 tW(round(numel(tW)/20))]);    % zoom-in a bit
    subplot(223); % magnitude plot
    hEigS1= plot(freq,db(rfft(yEPr)),'b','LineWidth',3); hold on; grid on; xlim(P.wDrive/(2*pi));
    hEigS2= plot(freq,db(rfft(yEPi)),'r--','LineWidth',1); hEigS3= plot(freq,db(rfft(yEMr)),'m.','MarkerSize',8);
    hEigS4= plot(freq,db(rfft(yEMi)),'g-.','LineWidth',1); ylabel('Magnitude [dB]'); xlabel('Frequency [Hz]');
    subplot(224); % phase plot
    hEigP1= plot(freq,angle(rfft(yEPr)),'b','LineWidth',3); hold on; grid on; xlim(P.wDrive/(2*pi));
    hEigP2= plot(freq,angle(rfft(yEPi)),'r--','LineWidth',1); hEigP3= plot(freq,angle(rfft(yEMr)),'m.','MarkerSize',8);
    hEigP4= plot(freq,angle(rfft(yEMi)),'g-.','LineWidth',1); ylabel('Magnitude [dB]'); xlabel('Frequency [Hz]');
end
...
if (1==0)
    ...
    clf; subplot(211); plot(t(1:frac),driveT(1:frac),'r'); ylabel('Sinusoidal drive'); grid on; hold on;
    subplot(212); plot(t(1:frac),yIvS(1:frac)); ylabel('Impulse response'); xlabel('Time [s]'); grid on;
end

% ++++++++++++++ [Fig.8] (re App.6) plot assoc. time series?
if (1==0)
    figure(8); clf; h1= plot(tS,X); hold on; grid on;
    h2= plot(tB,noiseI,'r--');
    h3= plot(tSS,xSS,'g.','MarkerSize',4);
...
if (1==0)
    ...
    figure(10); clf;
    subplot(211); h10a= plot(tW,yIvS,'b-.','LineWidth',2); grid on; hold on;
    ...
    hOW= M*(exp(c1*tW)- exp(c2*tW));  % O&W impulse response (eqn.4.175)
    subplot(211); h10b= plot(tW,hOW/(2*P.wo),'r-','LineWidth',1);
    ...
    for nn=1:numel(psiV)
        ...
        hOWv= Mv*(exp(c1v*tW)- exp(c2v*tW));
        % Note kludge factor of x0.5 and +2 below
        if (psiV(nn)==1), sOWv= 1- exp(-P.wo*tW)- P.wo*tW.*exp(-P.wo*tW) ;
        else    sOWv= 0.5*(2+ Mv*( (1/c1v)*exp(c1v*tW) - (1/c2v)*exp(c2v*tW) )); end
        subplot(211); h10m(nn)= plot(tW,hOWv/(2*P.wo),'-','Color',0.7*nn/numel(psiV)*[1 1 1]);
        subplot(212); h10s(nn)= plot(tW,sOWv,'-','Color',0.7*nn/numel(psiV)*[1 1 1]);
    end
...
if (1==0)
    ...
    magT11a= P.A./sqrt((P.wo^2-wDT.^2).^2 + ((P.gamma*wDT).^2));
    phaseT11a= atan((P.gamma*wDT)./(P.wo^2-wDT.^2));
    figure(11); clf; subplot(211); h11a= plot(wDT/P.wo,magT11a,'b-.','LineWidth',2);
    hold on; grid on; ylabel('Amplitude [arb]');
    subplot(212); h11b= plot(wDT/P.wo,unwrap(2*phaseT11a)/(4*pi),'b-.','LineWidth',2); % kludge to get unwrapping working
    ...
    magT11b= P.A./sqrt((P.wo^2-wDT.^2).^2 + ((P.gamma*wDT).^2));  % same as App.2
    phaseT11b= atan((P.gamma*wDT)./(-P.wo^2+wDT.^2));              % same as App.2
    subplot(211); h11c= plot(wDT/P.wo,magT11b,'r-','LineWidth',1);
    subplot(212); h11d= plot(wDT/P.wo,unwrap(2*phaseT11b)/(4*pi),'r-','LineWidth',1);
...
% ++++++++++++++ [Fig.12] autocorelation of impulse response/noise-driven
% case
...
if (1==0)
    ...
    acorr1= xcorr(yIvS,yIvS);  % autocrrelation of impulse response (re App.3)
    acorr2= xcorr(xSS(1:end-1),xSS(1:end-1));  % autocrrelation of "steady-state" noise-driven position (App.6)
    ...
    subplot(212); h12c= plot(2*pi*freq/P.wo,mag12A,'b.-');
    hold on; grid on; title('Fourier transform of (half of) above');
    h12d= plot(2*pi*freq/P.wo,db(rfft(acorr2S/max(acorr2S))),'r--');
    xlabel('Norm. frequency (w/w0)'); ylabel('Amplitude (normalized)'); xlim([0 2]);
    h12e= plot(wDT/P.wo,db(magT)+max(mag12A),'g-','LineWidth',2); % note kludge vertical shift
...
% magT* exp(i*phaseT) = (P.A)/((i*w-lambdaP)*(i*w-lambdaM))"
if (1==0)
    sig1= magT.*exp(i*phaseT);
    sig2= (P.A)./((i*wDT-lambdaP).*(i*wDT-lambdaM));
    figure(14); clf;
    subplot(211); h14a= plot(wDT/P.wo,abs(sig1),'b--','LineWidth',2);
    hold on; grid on; ylabel('Amplitude [arb]'); h14b= plot(wDT/P.wo,abs(sig2),'r-','LineWidth',1);
    subplot(212); h14c= plot(wDT/P.wo,unwrap(2*angle(sig1))/(4*pi),'b--','LineWidth',2); % kludge to get unwrapping working
    hold on; grid on;  xlabel('Normalized (angular) angular freq (w/wo)'); ylabel('(unwrapped) Phase [cycs]');
    h14d= plot(wDT/P.wo,unwrap(2*angle(sig2))/(4*pi),'r-','LineWidth',1);
